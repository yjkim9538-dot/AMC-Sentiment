...
### A. 운용사 직접 제출 (비밀 링크 · Netlify 배포 기준 권장)

1. 관리자(공제회)가 **제출 관리** 탭에서 **접수 회차**를 설정하고(예: `2026-3Q`), 운용사별 **업로드 링크**(`/u/<토큰>`)를 발급해 각 운용사에 전달.
2. 운용사는 링크로 접속(로그인 불필요) → 엑셀 양식 다운로드 → 작성 후 업로드. **본인 파일 업로드와 본인 제출 이력만** 볼 수 있고, 다른 운용사 데이터·통합 대시보드는 볼 수 없음.
3. 저장 시 운용사명은 **서버가 토큰 기준으로 강제** — 파일 안에 다른 운용사명이 적혀 있어도 발급된 운용사 명의로만 저장됨.
4. 관리자는 **제출 관리** 탭의 **전체 업로드·수정 이력**에서 모든 운용사의 제출·수정 내역(내용 포함)을 열람. 링크 유출 시 **재발급**으로 즉시 무효화.

### B. 관리자 일괄 업로드 (기존 방식 — 병행 가능)
...
| 제출 관리 | (관리자) 접수 회차 설정 · 운용사 업로드 링크 발급/재발급/삭제 · 전체 업로드·수정 이력 열람 |
| `/u/<토큰>` | (운용사 전용) 로그인 없이 본인 파일 업로드 + 본인 제출 이력만 조회 |
...
| POST | `/api/submissions` | 회차+운용사별 제출 저장(업서트) — 관리자 일괄 업로드용 |
| GET/POST | `/api/portal` | (Netlify) 운용사 포털 — 토큰 인증, 본인 이력 조회/제출(운용사명 서버 강제) |
| GET/POST | `/api/admin` | (Netlify) 관리자 — 접수 회차 설정·링크 발급/재발급/삭제·전체 이력 |
