...
4. **항상 실행 유지** — 다음 중 하나로 부팅 시 자동 실행:
   - **작업 스케줄러**: 트리거 "시스템 시작 시" → 동작 "프로그램 시작" 에 `server\start.bat` 지정.
   - 또는 `pm2`(`npm i -g pm2` → `pm2 start server.js --name amc` → `pm2 save`), 또는 `nssm` 으로 Windows 서비스 등록.
